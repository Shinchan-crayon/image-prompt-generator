...
def load_config(raw_config: dict, provider_spec: dict) -> dict:
    api_key = require_text(raw_config.get("api_key"), "ThinkAI API Key")
    base_url = require_text(raw_config.get("base_url"), "ThinkAI API 地址").rstrip("/")
    model = require_text(raw_config.get("model"), "ThinkAI 模型")
    if base_url != provider_spec["base_url"]:
        raise RuntimeError("ThinkAI 地址与固定契约不一致，请重新配置。")
    expected_model = provider_spec["models"][provider_spec["recommended_model"]]
    if model != expected_model:
        raise RuntimeError("ThinkAI 模型与固定契约不一致，请重新配置。")
    return {
        "provider": "thinkai",
...
def extract_image_source(response_json: dict):
    ...
    if not image_url:
        raise RuntimeError("ThinkAI 响应未包含 data[0].url。")
    if not image_url.startswith(("https://", "http://", "data:image/")):
        raise RuntimeError("ThinkAI 返回的图片 URL 协议无效。")
