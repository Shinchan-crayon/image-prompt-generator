...
批量表示全部 Prompt 审核完成后统一触发，并以受控并发执行。脚本默认最多同时发送三个
图片请求：
...
`--max-workers` 范围为 `1` 到 `8`，默认 `3`。设为 `1` 时按规划顺序串行生成。

同一任务同时只能有一个生成执行器。第二个并发调用必须直接拒绝，不得等待后再次发送，
也不得与第一个执行器共同推进状态。并发只发生在单个执行器内部。

任务按封面在前、正文配图在后的规划顺序提交。每个并发槽位空闲后才提交下一项，每张完成
后立即保存本地路径和状态。恢复时跳过所有已经成功的图片，不重复调用付费接口。
...
- 明确失败：记录 `failed`，不再提交尚未开始的项目。
- 超时、断线、响应无法解析或生成后下载失败：记录 `uncertain`，不再提交尚未开始的项目。
- 失败发生时已经发送的并发请求无法取消，必须等待其结束并逐项记录成功、失败或不确定状态。
