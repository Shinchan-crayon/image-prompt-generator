...
Image Prompt Generator Codex Plugin 内置的运行 Skill，用于把科技新闻、AI 行业分析、产品评测、商业洞察、
教程和教育内容转化为专业图片提示词。
...
配图，再逐张审核 Prompt；只有所有保留 Prompt 都批准后，才受控并发统一生图。
...
- 支持整篇文章自动规划多张图、逐张 Prompt 审核、全量批准门禁、受控并发、断点恢复和批量交付。

## 安装

本 Skill 由 Image Prompt Generator Plugin 安装，不需要单独复制到
`~/.codex/skills/`。从 GitHub 添加 marketplace 并在 Codex 插件页面安装：

```bash
codex plugin marketplace add Shinchan-crayon/image-prompt-generator
```

安装后请在新任务中使用 `$image-prompt-generator`。

安装生图连接器所需的 Python 依赖：

```bash
cd <插件安装目录>/skills/image-prompt-generator
...
安装后可以直接让 Codex Agent 列出固定的 5 个正式渠道入口；Agent 会在当前插件 Skill
目录运行：
...
`--max-workers` 可设为 `1` 到 `8`，默认 `3`；设为 `1` 时按规划顺序串行生成。
